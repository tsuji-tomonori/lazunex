...
- 変更系 API として `Idempotency-Key` を前提にし、`idempotency_records` で二重作成を防ぐ。
...
9. `idempotency_records` を作成または確認する。
10. `access_request.created` イベントを追記する。
11. `audit_events` を追記する。
12. 作成した申請情報を返す。
...
- `Idempotency-Key` による二重作成防止と `idempotency_records` 保存 SQL を実装する。
...
- Project 取得、公開 API 確認、reviewer 取得、active subscription 重複確認、pending request 重複確認は生成済み query で実行する。
- `api_access_requests`、`idempotency_records`、`access_request_events`、`audit_events` への保存を実装済み。
- request context は `X-Correlation-Id`、source IP、`User-Agent`、actor type を保持し、event/audit 記録に使う。
- 現状の idempotency は成功レスポンス snapshot の保存までで、既存 record の read/replay は未接続。
