# 07. SQL と `generated/queries.py` 生成

SQL は operation の `sql/` に置き、`src/tools/generate_queries.py` が `generated/queries.py` を生成する。現行 SQL は MySQL 方言、`@name` placeholder、table alias を使う。
...
- SELECT 句は取得列名を列挙する。
- 生成後の `generated/queries.py` は Pydantic params/row model と query function を持つ。
...
- SQL 生成物を operation 直下の `queries.py` 本体へ戻さない。
...
| SQL-DO-005 | MUST | `queries_generated_marker` | - | 生成後の `generated/queries.py` は Pydantic params/row model と query function を持つ。 |
...
| SQL-DONT-003 | MUST NOT | `generated_queries_layout` | - | SQL 生成物を operation 直下の `queries.py` 本体へ戻さない。 |
