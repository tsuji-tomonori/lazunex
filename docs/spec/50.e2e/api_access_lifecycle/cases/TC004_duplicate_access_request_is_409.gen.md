...
| Step 9 | `setup pending access request` | 重複申請確認のため、同一Project/API stageのPENDING申請を事前に用意する。 | 仕様どおりのHTTP status/body | - |
| Step 10 | `POST /projects/{projectId}/api-access-requests` | project_AからAPI_Aに対する利用申請を作成する。 | 仕様どおりのHTTP status/body | accessRequestId[${project.id},${api.id}] |
