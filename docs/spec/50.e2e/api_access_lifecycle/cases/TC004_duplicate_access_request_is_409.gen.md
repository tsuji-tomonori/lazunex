...
| Step 9 | `setup pending access request` | requestedAuthMode=BOTH (`both_auth_mode`) | 重複申請確認のため、同一Project/API stageのPENDING申請を事前に用意する。 | 仕様どおりのHTTP status/body | - |
| Step 10 | `POST /projects/{projectId}/api-access-requests` | requestedAuthMode=BOTH (`both_auth_mode`) | project_AからAPI_Aに対する利用申請を作成する。 | 仕様どおりのHTTP status/body | accessRequestId[${project.id},${api.id}] |
...
|---|---|---|---|---|---|---|
| E1 | Project作成確認 | Project作成API呼び出し後 | GET /projects のレスポンス | `GET /projects?keyword=${project.defaults.projectCode}` | project_A が検索結果に表示され、derivedState=ACTIVEである。 | `TC004_E_project_search_project_A.json` |
| E2 | public client更新確認 | Project public client更新API呼び出し後 | GET /projects/{projectId} のレスポンス | `GET /projects/{projectId}` | callback/logout URLが更新され、承認済みscopeが保持されている。 | `TC004_E_project_public_client_project_A.json` |
| E3 | 利用申請作成確認 | 利用申請API呼び出し後 | GET /projects/{projectId}/api-access-requests のレスポンス | `GET /projects/{projectId}/api-access-requests` | project_A × API_A の利用申請がPENDINGで表示される。 | `TC004_E_access_request_pending_project_A_API_A.json` |
