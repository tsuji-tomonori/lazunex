<!-- This file is generated by uv run python -m tools.generate_e2e_scenarios. Do not edit manually. -->

# TC002 reject request and no subscription

## 1. ケース概要

| 項目 | 内容 |
|---|---|
| Flow | `api_access_lifecycle` |
| Tier | `smoke_sandbox` |
| 目的 | 利用申請を却下し、subscriptionとAWS反映が作成されないことを確認する。 |
| 終了条件 | reject_api_access_request |

## 2. 選択要因

| Factor | Element |
|---|---|
| `F001` | provider + project owner + reviewer |
| `F002` | 有効なmanagement token |
| `F010` | 成功 |
| `F020` | 成功 |
| `F030` | 新規成功 |
| `F040` | reject |

## 3. 事前条件

- Cognito管理API用tokenを取得できる。
- Runtime対象のAPI Gateway REST APIがsandbox環境に存在する。
- テストデータの `apiCode` と `projectCode` はケースIDを含めて一意にする。
- `${project_api_key}`、`${confidential_client_secret}`、`${runtime_access_token}` はplaceholderとして扱う。

## 4. API呼び出し手順

### Step 1: POST /apis

Request:

```http
POST /apis
Authorization: Bearer ${management_or_runtime_token}
Idempotency-Key: ${case_id}-post_apis
Content-Type: application/json
```

OK条件:

- `${apiId}` と `${apiStageId}` を後続stepへ渡す。
- secret値、API key値、client secret値の実値をMarkdownやログに出さない。

### Step 2: POST /projects

Request:

```http
POST /projects
Authorization: Bearer ${management_or_runtime_token}
Idempotency-Key: ${case_id}-post_projects
Content-Type: application/json
```

OK条件:

- `${projectId}` と `${project_api_key}` を後続stepへ渡す。
- secret値、API key値、client secret値の実値をMarkdownやログに出さない。

### Step 3: POST /projects/${projectId}/api-access-requests

Request:

```http
POST /projects/${projectId}/api-access-requests
Authorization: Bearer ${management_or_runtime_token}
Idempotency-Key: ${case_id}-post_api_access_requests
Content-Type: application/json
```

OK条件:

- `${accessRequestId}` を審査stepへ渡す。
- secret値、API key値、client secret値の実値をMarkdownやログに出さない。

### Step 4: POST /api-access-requests/${accessRequestId}/reject

Request:

```http
POST /api-access-requests/${accessRequestId}/reject
Authorization: Bearer ${management_or_runtime_token}
Idempotency-Key: ${case_id}-reject_api_access_request
Content-Type: application/json
```

OK条件:

- reviewがREJECTEDとなり外部provisioningが発生しないことを確認する。
- secret値、API key値、client secret値の実値をMarkdownやログに出さない。

### Step 5: GET /projects/${projectId}/subscriptions

Request:

```http
GET /projects/${projectId}/subscriptions
Authorization: Bearer ${management_or_runtime_token}
Idempotency-Key: ${case_id}-get_project_subscriptions
Content-Type: application/json
```

OK条件:

- 期待するsubscription有無とderivedStateを確認する。
- secret値、API key値、client secret値の実値をMarkdownやログに出さない。

## 5. 後続確認

- reviewがREJECTEDになる。
- subscriptionが作成されずRuntime API呼び出しを実行しない。

## 6. クリーンアップ

- sandboxで作成したAPI catalog、project、Cognito client、Usage Plan、API keyはcleanup手順に従って削除または隔離する。
