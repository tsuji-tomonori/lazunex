...
| カラム | 型 | NULL許可 | キー | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| `access_request_id` | `UUID` | NO | PK | 利用申請ID。 |
| `project_id` | `UUID` | NO | FK -> projects(project_id) | 申請元Project ID。 |
| `api_id` | `UUID` | NO | FK -> apis(api_id) | 申請対象API ID。 |
| `api_stage_id` | `UUID` | NO | FK -> api_gateway_stages(api_stage_id) | 申請対象API stage ID。 |
