...
| カラム | 型 | NULL許可 | キー | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| `usage_plan_api_stage_id` | `UUID` | NO | PK | Usage Plan stage紐づけID。 |
...
| `subscription_id` | `UUID` | NO | FK -> project_api_subscriptions(subscription_id) | 紐づく利用権ID。 |
| `api_stage_id` | `UUID` | NO | FK -> api_gateway_stages(api_stage_id) | 紐づくAPI stage ID。 |
| `apigw_rest_api_id` | `VARCHAR(128)` | NO |  | API Gateway REST API ID。 |
| `apigw_stage_name` | `VARCHAR(128)` | NO |  | API Gateway stage名。 |
| `provisioned_at` | `TIMESTAMPTZ` | YES |  | API Gatewayへ反映した日時。 |
