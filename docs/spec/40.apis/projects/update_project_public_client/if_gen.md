...
| `400` | public app client更新リクエストが業務ルールに合わない場合。 | `application/json` | 14 field(s) |
| `401` | 認証情報が未指定、期限切れ、または検証できない場合。 | `application/json` | 14 field(s) |
| `403` | 呼び出し元が対象Projectのownerではない場合。 | `application/json` | 14 field(s) |
| `404` | 指定されたProjectまたはpublic app clientが存在しない場合。 | `application/json` | 14 field(s) |
| `409` | expected row versionが現在のversionと一致しない場合。 | `application/json` | 14 field(s) |
| `422` | path、header、bodyがOpenAPIスキーマの型や制約に一致しない場合。 | `application/json` | 14 field(s) |
| `429` | 呼び出し頻度が許可された上限を超えた場合。 | `application/json` | 14 field(s) |
| `500` | Lazunex内部で想定外のエラーが発生した場合。 | `application/json` | 14 field(s) |
| `502` | Cognitoへの反映で失敗応答を受け取った場合。 | `application/json` | 14 field(s) |
| `503` | Cognitoが一時的に利用できない場合。 | `application/json` | 14 field(s) |
...
| `error.details[].resource.projectId` | `string \| null` | no | 更新対象Projectの存在確認、権限確認、状態確認に使用するProject IDです。 | - |
| `error.details[].resource.expectedRowVersion` | `integer \| null` | no | 楽観ロック競合時に現在値との差分確認と再送判断に使用する期待行versionです。 | minimum=1 |
| `error.details[].resource.idempotencyKey` | `string \| null` | no | 同じpublic client更新リクエストの結果確認と再送に使用するIdempotency-Keyです。 | minLength=1, maxLength=256 |
    ...
    "details": [
      {
        ...
        "resource": {
          "projectId": "cb62b5f6-0000-0000-0000-000000000001",
          "expectedRowVersion": 3,
          "idempotencyKey": "update-public-client-001"
        }
      }
    ],
