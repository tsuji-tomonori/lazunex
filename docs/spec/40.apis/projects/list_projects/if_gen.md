...
| `derivedState` | `ProjectDerivedState \| null` | no | イベント履歴から導出した対象リソースの現在状態です。 | ACTIVE=プロジェクトが利用可能な状態です。 |
...
| `403` | 呼び出し元にProject一覧を参照する権限がない場合。 | `application/json` | 14 field(s) |
| `422` | queryがOpenAPIスキーマの型や制約に一致しない場合。 | `application/json` | 14 field(s) |
...
| `error.details[].resource.derivedState` | `ProjectDerivedState \| null` | no | 一覧復帰時に同じ絞り込みを再現するためのProject状態条件です。 | ACTIVE=プロジェクトが利用可能な状態です。 |
| `error.details[].resource.keyword` | `string \| null` | no | 一覧復帰時に同じ絞り込みを再現するための検索キーワードです。 | minLength=1, maxLength=200 |
| `error.details[].resource.ownerPrincipalId` | `string \| null` | no | 一覧復帰時に同じ絞り込みを再現するためのProject所有者IDです。 | minLength=1, maxLength=256 |
...
```json
{
  "error": {
    ...
    "details": [
      {
        "reason": "呼び出し元にProject一覧を参照する権限がない場合。",
        "statusCode": 403,
        "retryable": false,
        "reference": "trc_01HZY6WJ7X4W9A0V7P9N2Q3R4S",
        "resource": {
