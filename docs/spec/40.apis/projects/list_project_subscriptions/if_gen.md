...
| `403` | 呼び出し元に対象Projectの利用権一覧を参照する権限がない場合。 | `application/json` | 12 field(s) |
| `422` | pathまたはqueryがOpenAPIスキーマの型や制約に一致しない場合。 | `application/json` | 12 field(s) |
...
| `error.details[].resource.projectId` | `string \| null` | no | 一覧取得対象Projectの存在確認、権限確認、再取得に使用するProject IDです。 | - |
...
```json
{
  "error": {
    ...
    "details": [
      {
        "reason": "呼び出し元に対象Projectの利用権一覧を参照する権限がない場合。",
        "statusCode": 403,
        "retryable": false,
        "reference": "trc_01HZY6WJ7X4W9A0V7P9N2Q3R4S",
        "resource": {
...
```json
{
  "error": {
    ...
    "details": [
      {
        "reason": "pathまたはqueryがOpenAPIスキーマの型や制約に一致しない場合。",
        "statusCode": 422,
        "retryable": false,
        "reference": "trc_01HZY6WJ7X4W9A0V7P9N2Q3R4S",
        "resource": {
