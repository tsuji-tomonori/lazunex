...
| `403` | 呼び出し元に対象Projectを参照する権限がない場合。 | `application/json` | 11 field(s) |
| `404` | 指定されたProjectが存在しない場合。 | `application/json` | 11 field(s) |
| `401` | 認証情報が未指定、期限切れ、または検証できない場合。 | `application/json` | 11 field(s) |
| `422` | pathがOpenAPIスキーマの型や制約に一致しない場合。 | `application/json` | 11 field(s) |
