...
| `403` | 呼び出し元に対象Projectを参照する権限がない場合。 | `application/json` | 12 field(s) |
| `404` | 指定されたProjectが存在しない場合。 | `application/json` | 12 field(s) |
| `422` | pathがOpenAPIスキーマの型や制約に一致しない場合。 | `application/json` | 12 field(s) |
| `429` | 呼び出し頻度が許可された上限を超えた場合。 | `application/json` | 12 field(s) |
| `500` | Lazunex内部で想定外のエラーが発生した場合。 | `application/json` | 12 field(s) |
...
| `error.details[].resource.projectId` | `string \| null` | no | 取得対象Projectの存在確認、権限確認、問い合わせに使用するProject IDです。 | - |
...
```json
{
  "error": {
    ...
    "details": [
      {
        "reason": "呼び出し元に対象Projectを参照する権限がない場合。",
        "statusCode": 403,
        "retryable": false,
        "reference": "trc_01HZY6WJ7X4W9A0V7P9N2Q3R4S",
        "resource": {
