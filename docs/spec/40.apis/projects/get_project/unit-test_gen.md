...
- 対象: 条件分岐: 呼び出し元が Project 詳細を参照できない場合。
...
| 要因 | 要素 | 期待観点 |
| --- | --- | --- |
| `F01` 条件分岐: 呼び出し元が Project 詳細を参照できない場合。 | 成立 | HTTP 403 error response: caller cannot view project<br>log message_id: getProject.caller_cannot_view_project<br>log summary: 呼び出し元がProject詳細を参照できないため、リクエストを拒否した。 |

### TC002

| 要因 | 要素 | 期待観点 |
| --- | --- | --- |
| `F01` 条件分岐: 呼び出し元が Project 詳細を参照できない場合。 | 不成立 | 条件不成立側または後続処理を継続する。 |
