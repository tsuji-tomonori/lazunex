...
| `403` | 呼び出し元に対象APIを参照する権限がない場合。 | `application/json` | 12 field(s) |
| `404` | 指定されたAPIが存在しない場合。 | `application/json` | 12 field(s) |
...
| `error.details[].resource.apiId` | `string \| null` | no | 取得対象APIの存在確認、公開状態確認、問い合わせに使用するAPI IDです。 | - |
...
```json
{
  "error": {
    ...
    "details": [
      {
        "reason": "呼び出し元に対象APIを参照する権限がない場合。",
        "statusCode": 403,
        "retryable": false,
        "reference": "trc_01HZY6WJ7X4W9A0V7P9N2Q3R4S",
        "resource": {
