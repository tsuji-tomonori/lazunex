...
async def get_api(
    ...
    try:
        ...
        if not await api_functions.is_viewable_api(api, caller):
            return await api_functions.build_caller_cannot_view_api_response(api_id, caller)
        return await api_functions.build_api_detail_response(api)
    except ROUTER_HANDLED_EXCEPTIONS as error:
        return await api_functions.build_router_error_response(api_id, caller, error)
