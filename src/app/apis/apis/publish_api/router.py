...
async def publish_api(
    ...
    try:
        ...
        if not await api_functions.has_api_publish_permission(validated_request, caller):
            return await api_functions.build_caller_cannot_publish_api_response(
                request,
                idempotency_key,
                caller,
                request_context,
        ...
        if not await api_functions.update_api_gateway_stage_registration(
            ...
            return await api_functions.build_api_gateway_stage_registration_invalid_response(
                request,
                idempotency_key,
                caller,
                request_context,
        ...
        if await api_functions.has_registered_api(validated_request, session):
            return await api_functions.build_api_already_registered_response(
                request,
                idempotency_key,
                caller,
                request_context,
