...
class OperationalLogContextSchema(BaseModel):
    api_status_code: int | None = Field(
        default=None,
        description="テスト用のHTTP status code説明です。",
    )
    error_code: str | None = Field(
        default=None,
        description="テスト用のerror code説明です。",
    )
...
""",
    ...
    assert "`M001` | `listProjects.request_succeeded` | API処理が正常終了した。" in rendered
...
""",
    ...
    message_refs = [
        (message.catalog_id, message.message_id, message.level) for message in catalogs[0].messages
    ]
    assert message_refs == [("M001", "listProjects.caller_cannot_list_projects", "WARNING")]
    assert (
        validate_catalogs(
            catalogs,
            strict=True,
