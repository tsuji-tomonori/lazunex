...
from tools.e2e_models import FLOW_STEPS, TARGET_CASES
...
def test_e2e_case_list_links_scenarios(tmp_path: Path) -> None:
    ...
    assert "## 5. Component coverage summary" in content
    assert "## 6. Project x API matrix" in content
    assert "## 7. Cases by component" in content
    assert "## 8. Appendix" in content
    ...
    assert "管理API呼び出し主体" not in content
